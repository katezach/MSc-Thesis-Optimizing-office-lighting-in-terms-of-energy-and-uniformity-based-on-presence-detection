...
def SAMO_COBRA_PhaseII(cobra):
    ...
    def updateInfoAndCounters(cobra, xNew, yNewEval, conNewEval, phase):
        ...
        # yNewEval = np.atleast_2d(yNewEval)
        # if cobra['Fres'].shape[1] != yNewEval.shape[1]:
        #     yNewEval = np.hstack([yNewEval, np.full((yNewEval.shape[0], cobra['Fres'].shape[1] - yNewEval.shape[1]), np.nan)])
